...
export default function PopoverNode1() {
  ...
  return (
    <div className="md:w-[550px] sm:w-full space-y-4 px-4 sm:px-2">
      {/* Title */}
      <div className="inline-block bg-popoverred text-red-600 text-sm font-medium px-4 py-1 ml-5 sm:ml-2 rounded-lg">
      ...
      <div className="grid grid-cols-3 gap-2 px-12 sm:px-4">
        {gridData.map((item, index) => (
          <div
            key={index}
            className="bg-popoverred text-red-600 text-sm font-medium px-4 py-1 rounded-lg text-left"
          >
            {item}
          </div>
        ))}
